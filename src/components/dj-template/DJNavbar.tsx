...
interface DJNavbarProps {
  logoText: string;
  socialLinks?: SocialLink[];
}

const DJNavbar: React.FC<DJNavbarProps> = ({ logoText, socialLinks = [] }) => {
  ...
  // Get platform-specific icon class and fallback SVG
  const getPlatformIcon = (platformName: string) => {
    const iconMap: { [key: string]: { class: string; svg: string } } = {
      'instagram': {
          ...
          <div className="px-2 pt-2 pb-3 space-y-1">
            {navItems.map((item) => (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => scrollToSection(item.href)}
                className={`text-gray-300 hover:text-white block px-3 py-2 rounded-md text-base font-medium transition-colors duration-200 w-full text-right ${rubik.className}`}
              >
                {item.label}
              </motion.button>
            ))}
